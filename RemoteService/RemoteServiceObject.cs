...
namespace RemoteService
{
    public class RemoteServiceObject : HttpHosting, IRemoteServiceObject
    {
        ...
        public string DoTransaction(Dictionary<string, string> obj)
        {
            string errorMsg = "";
            try
            {
                ...
                foreach (var o in obj) {
                    var oo = JsonConvert.DeserializeObject<List<JObject>>( o.Value)
                        .Select(x => x.ToObject<Dictionary<string, object>>()).ToList();
                    dict.Add(o.Key, oo);
                }
                var model = (new MESDAL()).DoTransaction(dict);
            }
            catch (Exception ex) {
                errorMsg = ex.Message;
            }
        ...
        public string DoExtremeSpeedTransaction(Dictionary<string, string> obj)
        {
            string errorMsg = "";
            try
            {
                ...
                var model = (new MESDAL()).DoExtremeSpeedTransaction(dict);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        public string DoExtremeSpeedTransactionByDbType(string sModel)
        {
            string errorMsg = "";
            try
            {
                var input = JsonConvert.DeserializeObject<IEnumerable<SqlModel>>(sModel);

                input =  input.Select(xx =>
                {
                    xx.Model = (xx.Model as JArray).Select(x => x.ToObject<Dictionary<string, object>>()).ToList();
                    return xx;
                }).ToList();

                if (input == null || input.Count() < 1)
                    throw new Exception("no Data");
                var model = (new MESDAL()).DoExtremeSpeedTransactionByDbType(input);
            }
            catch (Exception ex)
            {
                errorMsg = ex.Message;
        ...
        private object DictionaryToDynamic(Dictionary<string, object> dict)
        {
        ...
        public int RowCount(string sSQL, Dictionary<string, object> param)
        {
            var model = (new MESDAL()).Query<dynamic>(sSQL, param);
        ...
        public string ExecuteQuery(string sql, Dictionary<string, object> obj)
        {
            DALModel ret = new DALModel { };
            try
            {
                var model = (new MESDAL()).Query<dynamic>(sql, obj);
                ret.IsSuccess = true;
                ret.Message = JsonConvert.SerializeObject(model);
            }
            catch (Exception ex) {
