using EDIWarehouseInOUT.WATCHWebReference;
using EDIWarehouseInOUT.WCF;
...
namespace EDIWarehouseInOUT
{
    public partial class fMain : Form
    {
        ...
        private void FeedbackMesStockResult(string strProduct, string strUrl, string strCarton) 
        {
            try
            {
                ...
                if (strProduct.Equals("WATCH"))
                {
                    ...
                    WATCHWebReference.MesApi ws = new WATCHWebReference.MesApi(strUrl);
                ...
                for (int i = 0; i < dtNoExistPartNO.Rows.Count; i++)
                {
                    if (strProduct.Equals("WATCH"))
                    {
                        ...
                        try
                        {
                            ...
                            WATCHWebReference.MesApi ws = new WATCHWebReference.MesApi(strFGinMESWcf);
        ...
        private void txtCartonOut_KeyDown(object sender, KeyEventArgs e)
        {
            //1.检查序号的有效性  function

            //2.检查序号是否能出库  SP

            //3.如果在DB中存在 则再界面显示箱的信息。

            //4.存储过程出库  SP  （自动出库需勾选，不勾选则就是检查 是否能出库）



        }

        private void chkAutoCartonOut_CheckedChanged(object sender, EventArgs e)
        {
            if (chkAutoCartonOut.Checked)
            {
                btnCartonOut.Visible = false;
                btnCartonOut.Enabled = false;
                txtCartonOut.Select();
                txtCartonOut.Focus();
            }
            else
            {
                btnCartonOut.Visible = true;
                btnCartonOut.Enabled = true;
            }
        }
    }
}
