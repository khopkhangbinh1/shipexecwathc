    ...
    public partial class fMain : Form
    {
            ...
            if (e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab)
            {
                ...
                string strSQL = string.Format(@"
                                    select a.customer_sn,
                                           ...
                                           c.region,
                                           b.SECURITY
                                      from NONEDIPPS.t_shipment_pallet b
                                      join NONEDIPPS.t_sn_status a
                                        on a.pack_pallet_no = b.pallet_no
                                      join NONEDIPPS.t_shipment_info c
                ...
                if (sDataSet.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        /// <summary>
                        /// HYQ：获取标准重量  NONEDIPPS.SP_WEIGHT_CHECKPALLETSTATUS(
                        ...
                        try
                        {
                            dt = ClientUtils.ExecuteProc("NONEDIPPS.SP_WEIGHT_CHECKPALLETSTATUS", procParams).Tables[0];
                        }
                        catch (Exception e1)
                        {
        ...
        private void insertPalletWeightLog(string shipmentid, string palletno, double weightvalue, string staweight, string dvalueweight, double upperweight, double lowerweight, string percentdec, string pass)
        {
            try
            {
                ...
                //insert into NONEDIPPS.t_pallet_weight_log
                ...
                string stringupdateweight = @"insert into NONEDIPPS.t_pallet_weight_log 
            ...
            catch (Exception ex)
            {
                ...
                LibHelperAC.MediasHelper.PlaySoundAsyncByConts();
        ...
        private void ComWeightSub()
        {
            ...
            //string strSQL = @"select a.customer_sn,a.pack_pallet_no,b.pallet_no,b.shipment_id from NONEDIPPS.t_sn_status a  join  NONEDIPPS.t_shipment_pallet b on a.pack_pallet_no =b.pallet_no where b.pallet_no='" + StrPalletNo + "'";
            ...
            if (sDataSet.Tables[0].Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    procParams[2] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "weightvalue", WeightValue };
                    procParams[3] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "dvalueweight", weightcvalue };
                    procParams[4] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "upperweight", UpperWeight };
                    procParams[5] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "lowerweight", LowerWeight };
                    ...
                    if ((WeightValue <= UpperWeight) && (LowerWeight <= WeightValue))
                    {
                        ...
                        try
                        {
                            dt = ClientUtils.ExecuteProc("NONEDIPPS.SP_WEIGHT_INSERTPALLETWEIGHT", procParams).Tables[0];
                        }
                        catch (Exception e1)
                        {
                            Show_Message(e1.ToString(), 0);
