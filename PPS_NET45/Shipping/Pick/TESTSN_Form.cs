    ...
    public partial class TESTSN_Form : Form
    {
        ...
        private bool PrintPalletLabel( int listrows, string pages)
        {
            ...
            if (dgvPC.Rows.Count > 0)
            {
                ...
                string strLabelPath = Application.StartupPath + "\\Shipping\\Label";
        ...
        private void txtPartNOSearch_KeyDown(object sender, KeyEventArgs e)
        {
            string strPartNo = txtPartNOSearch.Text.Trim();
            strPartNo = strPartNo.Trim().ToUpper();
            if (e.KeyCode != Keys.Enter)
            { return; }
            if (string.IsNullOrEmpty(strPartNo))
            {
                MessageBox.Show("输入的料号不能为空！");
                txtPartNOSearch.SelectAll();
                return;
            }
            //MPN
            cmbPartno.DataSource = null;
            ...
            string strSql = string.Format(@"select trim(part|| '*' || custpart || '*' || packqty)  as MPN  
                                    from pptest.oms_partmapping 
                                where (isdisabled <> 1
                                    or isdisabled is null )
                                        and part like '%{0}%'
