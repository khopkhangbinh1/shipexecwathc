...
namespace PickList
{
    public partial class PickListLabel : Form
    {
        ...
        {
            cmbSmid.Items.Clear();
            string strSql = @"SELECT DISTINCT SHIPMENT_ID FROM PPSUSER.T_order_INFO WHERE STATUS IN ('WP','IP') and  person_flag='Y'
                            and shipment_id in(select shipment_id from ppsuser.t_shipment_info where status in('WP','IP'))
        ...
        private void InitComboBoxPallet(String strShipmentID)
        {
            ...
            string strSql = @"SELECT tsp.PALLET_NO FROM PPSUSER.T_SHIPMENT_PALLET tsp where Shipment_id='"+strShipmentID+"'";
        ...
        order by aa.location_no asc, aa.trolley_line_no asc
             ...
             *  string strSQL = string.Format(@"
                         ...
                         group by aa.SHIPMENT_ID,
                                  ...
                                  aa.TROLLEY_LINE_NO,
                                  aa.TDM
            ", this.cmbSmid.Text.Trim(),this.cmbPallet.Text.Trim());
             */
            #endregion
