...
namespace PickList
{
    class PickPalletLabel
    {
        ...
        //PALLET_ID,MPN,ICTPN,LOCATIONID,QTY,CARTONQTY,
        ...
        public bool PrintPickPalletLabel(string strPickpalletno)
        {
            ...
            string strLabelPath = Application.StartupPath + "\\Shipping\\Label";//@"D:\MES_CLIENT\Shipping\Label";
        ...
        public bool PrintPickPalletLabel_new(string strPickpalletno)
        {
            ...
            //HYQ： 这部分是写.dat文件。 5个栏位就好
            string LabelParam = @"SHIPMENT_ID|PICK_PALLET_NO|CARRIER|QTY|SHIPMENT_TYPE|QTY1|QTY2|PACKCODEDESC|SIDTYPE|";
            ...
            //        + "          from(select distinct packcode,PALLETLENGTHCM|| '*' || PALLETWIDTHCM as remark from ppsuser.vw_mpn_info) "
            //        + "        group by packcode) f "
            ...
            //                           c.remark,c.type,
            //                           sum(qty) as sumqty
            //                      from (select a.shipment_id,e.type,
            //                                   b.pick_pallet_no,
            //                                   (select distinct scaccode
            //                                      from pptest.oms_carrier_tracking_prefix d
            //                                     where trim(d.carriercode) = e.carrier_code
            ...
            string strSIDType = string.Empty;
            if (dt.Rows.Count == 1)
            {
                string str1 = dt.Rows[0]["shipment_id"].ToString();
                string str2 = dt.Rows[0]["pick_pallet_no"].ToString();
