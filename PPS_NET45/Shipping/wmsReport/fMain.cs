...
namespace wmsReport
{
    public partial class fMain : Form
    {
        ...
        private DataTable findData()
        {
            ...
            if (cmbTYPE.Text == "储位检查报表")
            {
                ...
                if (cmbWHID.Text.Trim() != "")
                {
                    isInput = true;
                    Array.Resize(ref sqlparams, sqlparams.Length + 1);
                    strSql += " and location_no in (select location_no  from ppsuser.t_location where warehouse_id = :warehouse " + ")";
                    sqlparams[iPara] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "warehouse", cmbWHID.SelectedValue };
                    iPara = iPara + 1;
                }
        ...
        private void btnCarInfo_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = wb.GetShipmentInfoDataTable();
            if ((dt == null) || (dt.Rows.Count <= 0))
            {
                MessageBox.Show("天未找到出货数据,请检查！");
                Ng();
            }
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (i > 0 && dt.Rows[i][0].ToString().Equals(dt.Rows[i - 1][0].ToString()))
                {
                    ...
                    if (p > 0 && i == dt.Rows.Count - 1)
                    {
                        ...
                        sheet.AddMergedRegion(new CellRangeAddress(start + 2, start + 2 + p, 9, 9));
            ...
            //{
            //    WMSBLL wb = new WMSBLL();
            //    DataTable dtTemp = wb.GetShipmentInfoDataTable();
            ...
            //    Excel.Range range = xlWorksheet.get_Range(xlWorksheet.Cells[1, 1], xlWorksheet.Cells[1, 9]);
            //    range.Font.Size = 20;
            //    range.Font.Bold = true;
            //    range.Merge(oMissing);
            //    range = xlWorksheet.get_Range(xlWorksheet.Cells[2, 1], xlWorksheet.Cells[2, 9]);
            //    range.Font.Size = 16;
            //    range.Font.Bold = true;
            //    string strCarNo = "";
            ...
            //            stringBuffer = strCarNo + "\t" + strShipmentID + "\t" + strType + "\t" + strRegion + "\t" + strUpload + "\t" + strPalletCount + "\t" + strStatus
            ...
            //                strPalletCount = dtTemp.Rows[i]["PALLETCOUNT"].ToString().Trim();
            //                strStatus = dtTemp.Rows[i]["STATUS"].ToString().Trim();
            //                stringBuffer = "" + "\t" + strShipmentID + "\t" + strType + "\t" + strRegion + "\t" + strUpload + "\t" + strPalletCount + "\t" + strStatus
            //                        + "\t" + "" + "\t" + "";
            //                System.Windows.Forms.Clipboard.SetDataObject("");
            //                System.Windows.Forms.Clipboard.SetDataObject(stringBuffer);
            //                ((Excel.Range)xlWorksheet.Cells[i + 3, 1]).Select();
            ...
            //                range.Merge(oMissing);
            //                range = xlWorksheet.get_Range(xlWorksheet.Cells[3, 1], xlWorksheet.Cells[i + 3, 9]);
            //                range.Font.Size = 12;
            //                range = xlWorksheet.get_Range(xlWorksheet.Cells[1, 1], xlWorksheet.Cells[i + 3, 9]);
