...
namespace wmsReport
{
    public partial class fPpartTransfer : Form
    {
        ...
        private void initLocationTo()
        {
            string strSql = "select distinct a.trolley_line_no id, a.trolley_line_no name "
                        + "   from ppsuser.t_trolley_line_info a "
                        + "   where trolley_no in (select trolley_no from ppsuser.t_location_trolley) "
            ...
            else if (rdoAdvise.Checked == true)
            {
                ...
                if (strResult.Equals("OK"))
                {
                    ...
                    else
                    {
                        ...
                        string strAdviseLocation = wb.GetCarLineLocation(strAdviseCarlineno);
                        DialogResult strResultA = MessageBox.Show("获得车行号:" + strAdviseCarlineno +"*"+ strAdviseLocation + ",是否开始转移作业？", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
