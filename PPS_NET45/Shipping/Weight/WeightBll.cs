...
namespace Weight
{
    public class WeightBll
    {
        ...
        public bool CheckExistSSCC18Label(string strPalletNo)
        {
            if (string.IsNullOrEmpty(strPalletNo)) { return false; }
            WeightDal wd = new WeightDal();
            DataSet dataSet = wd.CheckExistSSCC18LabelBySQL(strPalletNo);
        ...
        public bool CheckPalletandSsccMatch(string strPalletNo, string strSSCC)
        {
            if (string.IsNullOrEmpty(strPalletNo) || string.IsNullOrEmpty(strSSCC)) { return false; }
            WeightDal wd = new WeightDal();
            DataSet dataSet = wd.CheckExistSSCC18LabelBySQL(strPalletNo, strSSCC);
            if (dataSet == null || dataSet.Tables[0].Rows.Count <= 0)
            {
                return false;
            }
        ...
        public void ShowShipmentPalletInfo( string palletno, DataGridView dtPallet)
        {
            ...
            if (dataSet.Rows.Count > 0)
            {
                for (int i = 0; i < dataSet.Rows.Count; i++)
                {
                    ...
                    dr.Cells[7].Value = dataSet.Rows[i]["SECURITY"].ToString();
                    dr.Cells[8].Value = dataSet.Rows[i]["栈板规格"].ToString();
                    try
                    {
        ...
        public string PPSCheckReprintRole(string inempno, string inwc, out string RetMsg)
        {

            WeightDal wd = new WeightDal();
            string strResult = wd.PPSCheckReprintRoleBySP( inempno,  inwc, out  RetMsg);
            if (strResult.Equals("NG"))
            {
                return "NG";
            }
        ...
        public string PPInsertRePrintLog(string strstation, string strlabelname, string strsn, string strppsempno, string strppsempname, string strreprintempno, string strreprintempname, string strreprintlogintime, string strcomputername, string strmac, string strremark, out string RetMsg)
        {
            WeightDal wd = new WeightDal();
            string strResult = wd.PPInsertRePrintLogBySP( strstation,  strlabelname,  strsn,  strppsempno,  strppsempname,  strreprintempno,  strreprintempname,  strreprintlogintime,  strcomputername,  strmac,  strremark, out  RetMsg);
            if (strResult.Equals("NG"))
            {
                return "NG";
            }
        ...
        //public string CheckReprintCarton(string strEmpNo, string strWC, string strCartonNo, out string RetMsg)
        //{
        //    WeightDal wd = new WeightDal();
        //    string strResult = wd.CheckReprintCartonBySP( strEmpNo,  strWC,  strCartonNo, out  RetMsg);
        //    if (strResult.Equals("NG"))
        //    {
        //        return "NG";
        //    }
