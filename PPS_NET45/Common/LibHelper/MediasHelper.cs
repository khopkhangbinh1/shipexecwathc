...
namespace LibHelper
{
    public partial class MediasHelper
    {
        ...
        /// <summary>
        /// Check 站 如果需要输入PackingList/DeliveryNote  A4 纸的时候 发出提示声音
        /// </summary>
        /// <returns></returns>
        public static bool PlaySoundAsyncByCheckDNPO()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\CheckDNPO.wav");
            return PlaySoundAsync(audioPath, false);
        }
        ...
        public static bool PlaySoundAsyncByUPS()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\upsremind.wav");
            return PlaySoundAsync(audioPath, false);
        }

        public static bool PlaySoundAsyncByFEDEX()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\fedexremind.wav");
            return PlaySoundAsync(audioPath, false);
        }

        public static bool PlaySoundAsyncBySF()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\sfremind.wav");
            return PlaySoundAsync(audioPath, false);
        }

        public static bool PlaySoundAsyncByDHL()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\dhlremind.wav");
            return PlaySoundAsync(audioPath, false);
        }
        public static bool PlaySoundAsyncByCheckOK()
        {
            string audioPath = string.Empty;
            string dllPath = Path.GetDirectoryName(new Uri(System.Reflection.Assembly.GetExecutingAssembly().CodeBase).LocalPath);
            audioPath = Path.Combine(dllPath, @"voice\checkok.wav");
            return PlaySoundAsync(audioPath, false);
        }
