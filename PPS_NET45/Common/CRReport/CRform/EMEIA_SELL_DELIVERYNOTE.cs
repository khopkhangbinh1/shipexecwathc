...
{
    public partial class EMEIA_SELL_DELIVERYNOTE
    {
        public EMEIA_SELL_DELIVERYNOTE(string deliveryNo, string palletNo)
        {
            Initialize(deliveryNo, palletNo, true, "");
        }

        public EMEIA_SELL_DELIVERYNOTE(string deliveryNo, string palletNo, bool print, string strPath)
        {
            Initialize(deliveryNo, palletNo, print, strPath);
        }
        public void Initialize(string deliveryNo, string palletNo, bool print, string strPath)
        {
            ...
            //                               round(SUM(QTY)*FGWEIGHTKG,2) AS TOTALNETWT,
            //                               ItemText
            ...
            //                         GROUP BY LINEITEM, MPN, UPC, PRODESC, FGWEIGHTKG, ItemText  ORDER BY LINEITEM ASC", deliveryNo);
            #endregion
            ...
            //                                   AND TOI.DELIVERY_NO = T9U.DELIVERYNO
            //                                   AND TOI.DELIVERY_NO = '{0}') T
            //                         GROUP BY LINEITEM, MPN,ICTPN, UPC, PRODESC, FGWEIGHTKG, ItemText  ORDER BY LINEITEM ASC", deliveryNo);
            #endregion
            ...
            string MlineSql = string.Format(@"SELECT LINEITEM,
                                      ...
                                      FROM (SELECT TOI.LINE_ITEM AS LINEITEM,
                                             ...
                                             WHERE TOI.ICTPN = VMI.ICTPARTNO
                                               ...
                                               AND TOI.DELIVERY_NO = '{0}') T
                                     GROUP BY LINEITEM, MPN, ICTPN, UPC, PRODESC, FGWEIGHTKG, ITEMTEXT, COO
            ...
            if (print)
            {
                Util.createRpt(Constant.EMEIA_SELL_DeliveryNote_URL, action);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.EMEIA_SELL_DeliveryNote_URL_ByGW, action, strPath);
            }
        }

        public EMEIA_SELL_DELIVERYNOTE(string deliveryNo, string palletNo, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
        ...
        private void Initialize2(string deliveryNo, string palletNo, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.EMEIA_SELL_DeliveryNote_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.EMEIA_SELL_DeliveryNote_URL_ByGW;
                }
            }
