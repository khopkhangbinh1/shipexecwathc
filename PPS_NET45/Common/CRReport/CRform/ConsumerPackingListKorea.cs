...
{
    public partial class ConsumerPackingListKorea
    {
        public string completeDiskPath = "";
        public ConsumerPackingListKorea(string deliveryNo, string lineItem, string shipmentId, bool print, string diskPath)
        {
            InitializeC(deliveryNo, lineItem, shipmentId, print, diskPath);
        }

        private void InitializeC(string deliveryNo, string lineItem, string shipmentId, bool print, string diskPath)
        {
            //初始化 DataSet
            DataSet ds = new DataSet();
            string headerSql = string.Format(@"select DISTINCT  t9u.ShiptoName as ST_NAME,
            ...
            if (print)
            {
                Util.CreateDataTable(Constant.KoreaPackingList_URL, ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.KoreaPackingList_URL_ByGW, ds, diskPath);
            }
        }
        public ConsumerPackingListKorea(string deliveryNo, string lineItem)
        {
        ...
        public ConsumerPackingListKorea(string deliveryNo, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
        ...
        public void Initialize2(string deliveryNo, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.KoreaPackingList_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.KoreaPackingList_URL_ByGW;
                }
            }
