...
{
    public partial class ConsolAMRLayoutMexico
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public ConsolAMRLayoutMexico(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            //AmrDsShippingLabel
            string headerSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_MEX_HEADER@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            string lineSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_MEX_LINE@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            ...
            string completePath = "";
            completePath = Application.StartupPath + "\\PDF\\" + shipID + "ConsolAMRMX.pdf";
