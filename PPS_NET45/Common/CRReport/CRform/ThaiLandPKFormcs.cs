...
{
    public partial class ThaiLandPKFormcs
    {
        /**
         ...
         *                 */
        public ThaiLandPKFormcs(string deliveryNo, string lineItem, string shipmentId, bool print)
        {
            InitializeC(deliveryNo, lineItem, shipmentId, print, "");
        }

        public ThaiLandPKFormcs(string deliveryNo, string lineItem, string shipmentId, bool print, string strPath)
        {
            InitializeC(deliveryNo, lineItem, shipmentId, print, strPath);
        }


        private void InitializeC(string deliveryNo, string lineItem, string shipmentId, bool print, string strPath)
        {
            //初始化 DataSet
            ...
            if (print)
            {
                Util.CreateDataTable(Constant.ThaiLandPK_URL, ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.ThaiLandPK_URL_ByGW, ds, strPath);
            }
        }

        public ThaiLandPKFormcs(string deliveryNo, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
        ...
        public void Initialize2(string deliveryNo, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.ThaiLandPK_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.ThaiLandPK_URL_ByGW;
                }
            }
            dtCrystal = ds;
            if (!isOnlyGetDS)
        ...
        public ThaiLandPKFormcs(string deliveryNo, string lineItem)
        {
            InitializeC2(deliveryNo, lineItem);
        }
        private void InitializeC2(string deliveryNo, string lineItem)
        {
            DataSet ds = new DataSet();
            string headerSql = string.Format(@"select t9u.ShiptoName as ST_NAME,
                                                        t9u.ShipToAddress as ST_ADDR1,
