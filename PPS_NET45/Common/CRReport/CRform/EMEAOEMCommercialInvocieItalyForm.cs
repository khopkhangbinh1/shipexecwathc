...
{
    public partial class EMEAOEMCommercialInvocieItalyForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEAOEMCommercialInvocieItalyForm(string shipID, string diskPath, bool print)
        {
        ...
        private void Initialize(string shipID, string diskPath, bool print)
        {
            ...
            String filePath = "";
            filePath = diskPath + shipID + "EMEAOEMCommercialItaly.pdf";
            diskCompletePath = filePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEACommercialItaly_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEACommercialItaly_URL, ds, filePath);
            }
        }
    }
}
