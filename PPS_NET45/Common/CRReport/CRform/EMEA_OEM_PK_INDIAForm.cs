...
{
    public partial class EMEA_OEM_PK_INDIAForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEA_OEM_PK_INDIAForm(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            string completePath = "";
            completePath = Application.StartupPath + "\\PDF\\" + shipID + "EMEA_OEM_PK_INDIA.pdf";
            diskCompletePath = completePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEAINDIA_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEAINDIA_URL, ds, completePath);
            }
        }
    }
}
