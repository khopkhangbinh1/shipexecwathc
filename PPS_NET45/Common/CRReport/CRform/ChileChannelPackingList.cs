...
{
    public partial class ChileChannelPackingList
    {
        public string completeDiskPath = "";
        public ChileChannelPackingList(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
        ...
        private void Initialize(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
            ...
            string headerSql = string.Format(@"select distinct b.shipment_id SHIPMENT_ID,
                                   ...
                                   a.soldtoaddress4 as SO_ADDR4,
                                   a.soldtocity || ' '||(CASE  WHEN INSTR(A.SOLDTOREGIONDESC, '=') > 0 THEN
                                                                   SUBSTR(CAST(A.SOLDTOREGIONDESC AS VARCHAR2(30)),
                                                                          1,
                                                                          INSTR(A.SOLDTOREGIONDESC, '=') - 1) || ' '
                                                                  ELSE
                                                                   ' '
                                                                END) || a.soldtozip as SO_ADDR5,
                                   ...
                                   'DAP' as FR_TERM, --US 的没有
                                   (SELECT SUM(TOI.QTY)  FROM   PPSUSER.T_ORDER_INFO   TOI  WHERE TOI.DELIVERY_NO = A.DELIVERYNO)  as TOT_QTY, --整个DN 的Piece  T_ORDER_INFO 里面Qty的Sum
                                   ...
                                   a.shiptoaddress4 as ST_ADDR4,
                                   a.shiptocity ||' '|| (CASE  WHEN INSTR(A.regiondesc, '=') > 0 THEN
                                                                   SUBSTR(CAST(A.regiondesc AS VARCHAR2(30)),
                                                                          1,
                                                                          INSTR(A.regiondesc, '=') - 1) || ' '
                                                                  ELSE
                                                                   ' '
                                                                END) || a.shiptozip as ST_ADDR5,
                                   ...
                                   o.packinglistname as RET_NAME1,
                                   ...
                                   a.ShipToContTel as RECE_C_PHONE 
...
and tsi.carrier_code = octp.carriercode
            ...
            #region  lineSql 20200213bk
            //string lineSql = string.Format(@" SELECT T.SHIPMENT_ID AS SHIPMENT_ID,
            ...
            if (print)
            {
               ...
               Constant.CHANNELCHILE_URL,
               ds);

            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.CHANNELCHILE_URL_ByGW, ds, diskPath);
            }



        }
        public ChileChannelPackingList(string acDn, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
...
and tsi.carrier_code = octp.carriercode
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.CHANNELCHILE_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.CHANNELCHILE_URL_ByGW;
                }
            }
