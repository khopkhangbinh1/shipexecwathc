...
{
    public partial class ConsolAMRLayoutBrazil
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public ConsolAMRLayoutBrazil(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            string headerSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_BZ_HEADER@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            string lineSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_BZ_LINE@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            ...
            string completePath = "";
            completePath = Application.StartupPath + "\\PDF\\" + shipID + "ConsolAMRLayoutBZ.pdf";
            diskCompletePath = completePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(
            Constant.ConsolAMRBR_URL,
            ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.ConsolAMRBR_URL, ds, completePath);
            }
        }
    }
}
