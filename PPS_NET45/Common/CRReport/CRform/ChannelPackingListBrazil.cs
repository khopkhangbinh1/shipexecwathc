...
{
    public partial class ChannelPackingListBrazil
    {
        public string completeDiskPath = "";
        public ChannelPackingListBrazil(string acDn, string lineItem, string shipmentid, bool print, string diskPath)
        {
        ...
        private void Initialize(string acDn, string lineItem, string shipmentid, bool print, string diskPath)
        {
            ...
            string headerSql = string.Format(@"select DISTINCT b.shipment_id SHIPMENT_ID,
                                   ...
                                   '' as FR_TERM, --US 的没有
                                   (SELECT SUM(TOI.QTY)  FROM   PPSUSER.T_ORDER_INFO   TOI  WHERE TOI.DELIVERY_NO = '{0}'and toi.shipment_id = '{2}')  as TOT_QTY, --整个DN 的Piece  T_ORDER_INFO 里面Qty的Sum
                                   ...
                                   a.shiptoaddress4 as ST_ADDR2,
                                    a.shiptocity || a.shiptostate || a.shiptozip as ST_ADDR3,
                                   '' as ST_ADDR4,
                                   '' as ST_ADDR5,
...
and tsi.carrier_code = octp.carriercode
            ...
            if (print)
            {
                ...
                Util.CreateDataTable(
                    Constant.CHANNELBRAZIL_URL,
                    ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.CHANNELBRAZIL_URL_ByGW, ds, diskPath);
            }


        }

        public ChannelPackingListBrazil(string acDn, string lineItem, string shipmentid, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
...
and tsi.carrier_code = octp.carriercode
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.CHANNELBRAZIL_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.CHANNELBRAZIL_URL_ByGW;
                }
            }
