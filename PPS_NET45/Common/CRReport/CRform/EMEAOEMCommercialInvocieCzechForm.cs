...
{
    public partial class EMEAOEMCommercialInvocieCzechForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEAOEMCommercialInvocieCzechForm(string shipID, string diskPath, bool print)
        {
        ...
        private void Initialize(string shipID, string diskPath, bool print)
        {
            ...
            String filePath = "";
            filePath = diskPath + shipID + "EMEAOEMCommercialCzech.pdf";
            diskCompletePath = filePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEACommercialCzech_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEACommercialCzech_URL, ds, filePath);
            }
        }
