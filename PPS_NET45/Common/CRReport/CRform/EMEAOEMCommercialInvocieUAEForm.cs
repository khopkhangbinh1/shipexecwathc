...
{
    public partial class EMEAOEMCommercialInvocieUAEForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEAOEMCommercialInvocieUAEForm(string shipID, string diskPath, bool print)
        {
        ...
        private void Initialize(string shipID, string diskPath, bool print)
        {
            ...
            String filePath = "";
            filePath = diskPath + shipID + "EMEAOEMCommercialUAE.pdf";
            diskCompletePath = filePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEACommercialUAE_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEACommercialUAE_URL, ds, filePath);
            }
        }
    }
}
