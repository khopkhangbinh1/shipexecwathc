...
{
    public partial class EMEA_OEM_PK_MITForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEA_OEM_PK_MITForm(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            string completePath = "";
            completePath = Application.StartupPath + "\\PDF\\" + shipID + "EMEA_OEM_PK_MIT.pdf";
            diskCompletePath = completePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEAMIT_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEAMIT_URL, ds, completePath);
            }
        }
    }
}
