...
{
    public partial class EMEA_OEM_PK_DirectShipForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEA_OEM_PK_DirectShipForm(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            string completePath = "";
            completePath = Application.StartupPath + "\\PDF\\" + shipID + "EMEA_OEMPKDS.pdf";
            diskCompletePath = completePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEADirectShip_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEADirectShip_URL, ds, completePath);
            }
        }
    }
}
