...
{
    public partial class EMEAOEMCommercialInvocieMITForm 
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEAOEMCommercialInvocieMITForm(string shipID, string diskPath, bool print)
        {
        ...
        private void Initialize(string shipID, string diskPath, bool print)
        {
            ...
            String filePath = "";
            filePath = diskPath + shipID + "EMEAOEMCommercialMIT.pdf";
            diskCompletePath = filePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEACommercialMIT_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEACommercialMIT_URL, ds, filePath);
            }
        }
    }
}
