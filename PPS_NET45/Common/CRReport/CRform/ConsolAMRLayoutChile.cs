...
{
    public partial class ConsolAMRLayoutChile
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public ConsolAMRLayoutChile(string shipID, bool print)
        {
        ...
        private void Initialize(string shipID, bool print)
        {
            ...
            string headerSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_CHI_HEADER@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            string lineSql = @"SELECT * FROM WMUSER.AC_AMR_DS_CPL_CHI_LINE@dgedi T0 WHERE T0.SHIPMENT_ID = '" + shipID + "'";
            ...
            if (print)
            {
                Util.CreateDataTable(
                  Constant.ConsolAMRChile_URL,
                  ds);

            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.ConsolAMRChile_URL, ds, completePath);
            }
        }
    }
}
