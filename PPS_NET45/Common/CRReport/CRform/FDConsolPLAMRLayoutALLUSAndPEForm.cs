...
{
    public partial class FDConsolPLAMRLayoutALLUSAndPEForm
    {
        public string diskCompletePath = "";
        public FDConsolPLAMRLayoutALLUSAndPEForm(string shipID, bool print)
        {
        ...
        public void Initialize(string shipID, bool print)
        {
            ...
            string headerSql = @"select distinct a.SHIPID_TT,
                                ...
                                a.REMARK,
                                a.DECLAR           
                                from WMUSER.AC_AMR_FD_CPL_US_PR_HEADER@dgedi a,
            ...
            string lineSql = @"select t0.so_no as SHIPID_TT,
                                ...
                                WMUSER.AC_AMR_FD_CPL_US_PR_LINE@dgedi t1,
                                ppsuser.g_ds_partinfo_t     d,
                                sajet.sys_part              e
            ...
            string shipDateSql = @"SELECT distinct ship_date FROM PPSUSER.G_SHIPPING_DETAIL_T WHERE so_no = '" + shipID + "'";
            string allPalletWgSql = @"select SUM(NVL(STANDARD_WEIGHT,0)) ALLPALLET_WEIGHT
                                        from ppsuser.g_palletedi_info A
                                        where A.SSCC IN (SELECT DISTINCT SSCC FROM PPSUSER.G_SHIPPING_DETAIL_T WHERE SO_NO='" + shipID + "')";
            string HAWB = "";
            DataTable headerDt = Util.getDataTaleC(headerSql, "Header");
            if (headerDt != null && headerDt.Rows.Count > 0)
            {
                HAWB = headerDt.Rows[0][15].ToString();
            }
            ...
            string tmp = "";
            tmp = Application.StartupPath + "\\PDF\\" + currentDate + " " + HAWB + " Import Customs Clearance Doc.CPL.pdf";
