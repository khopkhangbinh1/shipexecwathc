...
{
    public partial class ChannelPackingListUS
    {
        public string completeDiskPath = "";
        /// <summary>
        /// Consol PL-AMR-Layout - US
        /// </summary>
        /// <param name="acDn">deliveryNo</param>
        /// <param name="lineItem"></param>
        /// <param name="print">打印PDF还是按照默认打印机打印</param>
        /// <param name="diskPath">水晶报表路径</param>
        public ChannelPackingListUS(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
        ...
        private void Initialize(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
                                                               ...
                                                               /*    o.shiplabelname as RET_NAME1,
                                                                   ...
                                                                   ( CASE WHEN (SELECT DISTINCT ORAM.packinglistname  FROM  PPTEST.OMS_RETURN_AMR ORAM
                                                                    WHERE ORAM.SHIPTOCNTYCODE = A.SHIPCNTYCODE
                                                                     and substr(a.shiptozip, 1, 3) >= ORAM.zipcodebegin
                                                                     and substr(a.shiptozip, 1, 3) <= ORAM.zipcodeend)IS NULL
                                                                    ...
                                                                    WHERE ORAM.SHIPTOCNTYCODE = A.SHIPCNTYCODE
                                                                     ...
                                                                     ELSE
                                                                      ( SELECT DISTINCT ORAM.packinglistname  FROM  PPTEST.OMS_RETURN_AMR ORAM
                                                                        WHERE ORAM.SHIPTOCNTYCODE = A.SHIPCNTYCODE
                                                                         and substr(a.shiptozip, 1, 3) >= ORAM.zipcodebegin
                                                                         and substr(a.shiptozip, 1, 3) <= ORAM.zipcodeend)
                                                                     END) AS RET_NAME1,
                                                                    -- RET_NAME2
            ...
            if (print)
            {
                Util.CreateDataTable(
                                          Constant.CHANNELUS_URL,
                                          ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.CHANNELUS_URL_ByGW, ds, diskPath);
            }
        ...
        public ChannelPackingListUS(string acDn, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
        ...
        private void Initialize2(string acDn, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.CHANNELUS_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.CHANNELUS_URL_ByGW;
                }
            }
