...
{
    public partial class FDAPACPackingListForm
    {
        public string diskCompletePath = "";
        public FDAPACPackingListForm(string so_no, string diskPath, bool print)
        {
        ...
        private void Initialize(string invoiceNumber, string so_no, string diskPath, bool print)
        {
            ...
            //AmrDsShippingLabel
            string headerSql = @"SELECT * FROM WMUSER.AC_FD_APAC_PL_HEADER@dgedi T0 WHERE T0.INVOICE_NO = '" + invoiceNumber + "'";
            ...
            string volumeSql = @"select sum(a.Volume) as volume
            ...
            ds.Tables.Add(Util.getDataTaleC(volumeSql, "Volume"));
            ...
            if (print)
            {
                Util.CreateDataTable(Constant.FDAPACPackingList_URL, ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.FDAPACPackingList_URL, ds, filePath);
            }
        }
    }
}
