...
{
    public partial class EMEAOEMCommercialInvocieTurkeyForm
    {
        public string diskCompletePath = ""; //全局变量返回pdf路径
        public EMEAOEMCommercialInvocieTurkeyForm(string shipID, string diskPath, bool print)
        {
        ...
        private void Initialize(string shipID, string diskPath, bool print)
        {
            ...
            String filePath = "";
            filePath = diskPath + shipID + "EMEAOEMCommercialTurkey.pdf";
            diskCompletePath = filePath; //全局变量返回pdf路径
            if (print)
            {
                Util.CreateDataTable(Constant.EMEACommercialTurkey_URL, ds);
            }
            else
            {
                Util.exportCRPDFAndSendEmail(Constant.EMEACommercialTurkey_URL, ds, filePath);
            }
            Util.printPDFCrystalReportV2(Constant.EMEACommercialTurkey_URL, ds, filePath);
        }
    }
}
