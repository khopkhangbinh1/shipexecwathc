...
{
    public partial class Ltr_PkList_MEX 
    {
        public string completeDiskPath = "";
        public Ltr_PkList_MEX(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
        ...
        private void Initialize(string acDn, string lineItem, string shipmentId, bool print, string diskPath)
        {
            ...
            string headerSql = string.Format(@"select distinct a.shiptoname as ST_NAME,
                                                ...
                                                (SELECT SUM(TOI_sum.QTY)
                                                  ...
                                                  and   toi_sum.shipment_id = toi.shipment_id
                                                  ) TOT_QTY,
                                                (   select round(sum(t1.totalWeight), 2)
                                                     from (select TOI.ICTPN,
                                                            ...
                                                            group by TOI.ICTPN, TOI.DELIVERY_NO, vmi.GROSSWEIGHTKG) t1) TOT_GW,
                                 ...
                                 where toi.delivery_no = ppsuser.t_newtrim_function(a.deliveryno)
                                 and   toi.line_item = ppsuser.t_newtrim_function(a.custdelitem)
            ...
            string lineSql = string.Format(@"   SELECT T.DELIVERY_NO as AC_DN,
                                                ...
                                                FROM (SELECT TOI.DELIVERY_NO,
                                                            ...
                                                            DECODE(t9u.MSG1 || t9u.MSG2 || t9u.MSG3 || t9u.MSG4 ||
                                                                    ...
                                                                    '',
                                                                    '',
                                                                    'Mensaje de Regalo') AS GIFT_MSG8,
                                                            toi.qty
                                                ...
                                                GROUP BY T.DELIVERY_NO,
                                                        ...
                                                        T.msg4,
                                                        T.msg5
                                                order by LINE_ITEM", acDn, shipmentId);
            ds.Tables.Add(Util.getDataTaleC(headerSql, "Header"));
            ds.Tables.Add(Util.getDataTaleC(lineSql, "Line"));
            if (print)
            {
                Util.CreateDataTable(Constant.Ltr_PkList_MEX_URL, ds);
            }
            else
            {
                Util.printPDFCrystalReportV2(Constant.Ltr_PkList_MEX_URL_ByGW, ds, diskPath);
            }
        }

        public Ltr_PkList_MEX(string acDn, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
        ...
        private void Initialize(string acDn, string lineItem, string shipmentId, string strCrystalFullPath, bool isCustom, bool isPDF, string strPDFPath, int nCopies, bool isOnlyGetDS, out DataSet dtCrystal, out string serverFullLabelName)
        {
            dtCrystal = null;
            ...
            if (string.IsNullOrEmpty(strCrystalFullPath))
            {
                ...
                {
                    strCrystalFullPath = Constant.Ltr_PkList_MEX_URL;
                }
                else//isCustom如果是关务的是用宋体的模板 
                {
                    strCrystalFullPath = Constant.Ltr_PkList_MEX_URL_ByGW;
                }
            }
            dtCrystal = ds;
            if (!isOnlyGetDS)
        ...
        public Ltr_PkList_MEX(string acDn, string lineItem)
        {
            Initialize2(acDn, lineItem);
        }
        private void Initialize2(string acDn, string lineItem)
        {
            DataSet ds = new DataSet();
            string headerSql = string.Format(@"select distinct a.shiptoname as ST_NAME,
                                                a.shiptoaddress as ST_ADDR1,
