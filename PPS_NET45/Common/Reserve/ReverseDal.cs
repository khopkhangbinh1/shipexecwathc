...
namespace Reverse
{
    public class ReverseDal
    {
        ...
        {
            string sql = @"SELECT * FROM ppsuser.edi940_header A 
                             LEFT JOIN ppsuser.g_ds_shipment_dn_t b 
        ...
        {
            string sql = @"SELECT * FROM ppsuser.g_sn_status 
        ...
        public int CheckCartonInPalletOfCartonHold(string cartonNo)
        {
            string sql = @"SELECT * 
                            ...
                            WHERE hold_flag = 'Y' 
                              AND carton_no IN (
                                    SELECT inputdata 
                                      FROM ppsuser.g_ds_pickinglist_t 
                                     WHERE pallet_no IN(
                                             SELECT pallet_no 
                                               FROM ppsuser.g_ds_pickinglist_t 
        ...
        public int CheckCartonInReadPalletHold(string palletNo)
        {
            string sql = @"SELECT * 
                            ...
                            WHERE hold_flag = 'Y' 
                              AND carton_no IN (
                                     SELECT inputdata 
                                       FROM ppsuser.g_ds_pickinglist_t a
        ...
        public int CheckCartonInShipmentHold(string shipmentId)
        {
            string sql = @"SELECT * 
                             FROM ppsuser.g_ds_pickinglist_t A
                             JOIN ppsuser.g_sn_status b ON A.inputdata = b.carton_no
                            WHERE b.hold_flag = 'Y' 
                                    ...
                                    UNION ALL 
                                     SELECT  DISTINCT 
                                       ...
                                       FROM ppsuser.g_sn_status a
                                            JOIN ppsuser.g_ds_pickinglist_t b 
                                              ON A.carton_no = b.inputdata 
                                       LEFT JOIN ppsuser.g_ds_shimment_base_t c
        ...
        {
            string sql = @"UPDATE ppsuser.edi940_header 
                              SET trans_flag ='N' 
                            WHERE trans_flag IN ('H','Z') 
                              AND ac_dn IN (
                                     SELECT dn 
                                       FROM ppsuser.g_ds_shipment_dn_t 
        ...
        {
            string sql = @"UPDATE ppsuser.g_sn_status 
                              SET hold_flag = 'N' 
        ...
        public DataTable GetInfoByCartonNo(string cartonNo)
        {
            string strSql = @"SELECT ROWNUM , D.* FROM (
                                     SELECT DISTINCT 
                                           ...
                                           ,'' AS status
                                       FROM ppsuser.g_sn_status A
                                       LEFT JOIN ppsuser.g_ds_scandata_detail_t b 
                                              ON A.serial_number = b.serial_number AND  A.carton_no = b.carton_no
                                            JOIN ppsuser.g_ds_scandata_t C 
                                              ON b.dn = C.dn 
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_pallet_t 
                                 SET  shipment_flag = '' 
                               WHERE shipment_id = :ShipmentId ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_shipment_ddline_t 
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_shipment_dnline_t 
                                 SET status = 'INPROCESS' 
                               WHERE shipment_id = :ShipmentId ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_weight_t 
                               WHERE shipment_id = :shipmentid ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_weightlist_t 
                               WHERE shipment_id = :shipmentid ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_scandata_t 
                                 SET check_flag = ''
                               WHERE shipment_id = :ShipmentId ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_pack_shipping_label 
                               WHERE shipment_id = :shipmentid ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_pack_shipping_label 
                                SET unship_flag = 'Y'
                              WHERE shipment_id = :ShipmentId 
                                AND pallet_no = :PalletNo ";
            object[][] parameterArray = new object[2][];
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_scandata_detail_t 
                               WHERE shipment_id = :shipmentid ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_scandata_t 
                               WHERE shipment_id = :shipmentid ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"DELETE ppsuser.ict_lps_line 
        ...
        public void DelLpsLine(string shipmentId, string palletNo)
        {
            string strSql = @"DELETE ppsuser.ict_lps_line 
                               WHERE mother_child_tag = 'C'
                                 AND carton_id IN (
                                       SELECT carton_no 
                                         FROM ppsuser.g_ds_scandata_detail_t 
                                        WHERE shipment_id = :ShipmentId
                                          AND pallet_no = :PalletNo )";
            object[][] parameterArray = new object[2][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
        ...
        {
            string strSql = @"UPDATE ppsuser.g_ds_pick_t 
                                 SET scan_qty = '0' 
                               WHERE shipment_id = :ShipmentId ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
        ...
        {
            string strSql = @"UPDATE ppsuser.g_sn_status A 
                                 SET A.shipping_id = '0'
                                    ,A.flag='0'
                                    ,A.pps_shipment = ''
                               ...
                               WHERE A.carton_no
                                  IN( SELECT inputdata 
                                        FROM ppsuser.g_ds_pickinglist_t b
                                       WHERE b.shipment_id = :ShipmentId )";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
        ...
        {
            string strSql = @"UPDATE ppsuser.g_sn_status 
                                 SET shipping_id = '0'
                                    ,flag='0'
                                    ,pps_shipment = ''
        ...
        {
            string strSql = @"DELETE ppsuser.g_ds_pickinglist_t 
                               WHERE shipment_id = :ShipmentId ";
            object[][] parameterArray = new object[1][];
            parameterArray[0] = new object[] { ParameterDirection.Input, OracleDbType.Varchar2, "ShipmentId", shipmentId };
            ClientUtils.ExecuteSQL(strSql, parameterArray);
