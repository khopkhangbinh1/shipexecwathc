            ...
            try
            {
                ...
                for (int j = 0; j < dt.Rows.Count; j++)
                {
                    ...
                    string total_cartonsSQL = string.Format(@"select sum(c.total_cartons)  as total_cartons,sum(c.empty_cartons) as empty_cartons
                                                            ...
                                                            where a.sscc = b.sscc
                                                            and b.so_no = '{0}'  and b.po_no = '{1}') c
                                                        ", so_no, dt.Rows[j]["ac_po"].ToString());
                    ...
                    string palletSql = string.Format(@"select count(1) as pallets
                                                ...
                                                from   ppsuser.g_palletedi_info t,ppsuser.g_shipping_detail_t a
                                                where t.so_no = '{0}' and a.po_no = '{1}'  and a.sscc = t.sscc
                                                group by t.SSCC) a", so_no, dt.Rows[j]["ac_po"].ToString());
                    ...
                    string gross_WeightSQL = string.Format(@"select sum(a.allpallet_weight) as Gross_Weight,sum(a.Volume) as volume
                                                    ...
                                                    where t.so_no = '{0}' and b.po_no = '{1}' and t.sscc = b.sscc) a
                                                    group by a.so_no", so_no, dt.Rows[j]["ac_po"].ToString());
                    DataTable gross_WeightDt = ClientUtils.ExecuteSQL(gross_WeightSQL).Tables[0];

                    //获取PacklingList列表的值
                    string pl_lineSQL = string.Format(@"select invoice_No ,ac_po, ac_pn ,ac_pn_desc, sum(qty) as qty
                                                from wmuser.AC_FD_AMR_PL_LINE@dgedi a where invoice_no = '{0}' and ac_po = '{1}'
                                                group by invoice_No ,ac_po, ac_pn ,ac_pn_desc order by AC_PO, AC_PN ", dt.Rows[0]["invoice_no"].ToString(), dt.Rows[j]["ac_po"].ToString());
                    ...
                    for (int i = 0; i < 10; i++)
                    {
                        if (i < pl_lineDt.Rows.Count)
                        {
                            ...
                            string qtySql = string.Format(@"select a.mpn,a.po_no,sum(a.qty) qty
                                ...
                                and a.mpn = '{1}'
                                and a.so_no = '{2}'
                                group by a.mpn,a.po_no", pl_lineDt.Rows[i]["ac_po"].ToString(), pl_lineDt.Rows[i]["ac_pn"].ToString(), so_no);
                    ...
                    sb.Append(dt.Rows[j]["invoice_no"].ToString() + "|"
                        + DateTime.Now.ToString("yyyy/MM/dd") + "|"
                        + dt.Rows[j]["st_name"].ToString() + "|"
                        + dt.Rows[j]["hawb"].ToString() + "|"
                        + dt.Rows[j]["ac_po"].ToString() + "|"
                        + qty + "|"
                        + total_cartons + "|"
                        + product_weight.ToString("F2") + "|"
                        + rough_weight.ToString("F2") + "|"
                        + pallets + "|"
                        + dt.Rows[j]["carr_code"].ToString() + "|"
                        + dt.Rows[j]["ship_per"].ToString() + "|"
                        + dt.Rows[j]["st_addr"].ToString() + "|"
                        + incotenms + "|"
                        + (j + 1).ToString() + "|"
                        + dt.Rows.Count + "|"
                        );
