...
namespace iMWS.Api.Controllers
{
    ...
    public class PPSPickController : ApiController
    {
            ...
            try
            {
                ...
                string sql = @"select aa.shipment_id   ShipmentID,
                      ...
                      from(select CASE
                                   ...
                                   b.pallet_no,
                                   a.priority,
                                   decode(b.pallet_type,'999','MIX','NO MIX') PalletType
                              from ppsuser.t_shipment_info a
                              join ppsuser.t_shipment_pallet b
                                on a.shipment_id = b.shipment_id
                               and a.status in ('WP', 'IP')
                ...
                PPSDb db = new PPSDb();
                var list = new List<PickTaskSummaryViewModel>();
                if (!string.IsNullOrEmpty(shipmentid))
                    list = db.Query<PickTaskSummaryViewModel>(sql, new { shipDate, shipmentid }).ToList();
                else
                    list = db.Query<PickTaskSummaryViewModel>(sql, new { shipDate }).ToList();
                return Ok(Result.Create(true, "", JsonConvert.SerializeObject(list)));
            }
            catch (Exception ex)
            {
                errmsg = ex.Message;
        ...
        public IHttpActionResult GetPickPalletInfo(string palletId)
        {
            ...
            string sql = @"
                select distinct a.shipment_id ShipmentId,
                       a.pallet_no,
            ...
            result = list.First();
            return Ok(Result.Create(true, "", JsonConvert.SerializeObject(result)));
        }
        ...
        public IHttpActionResult GetPickTaskItem(string palletId, string area)
        {
            ...
            string sql = @"SELECT C.ICTPN ICTPN, C.QTY QTY, C.CARTON_QTY CTNQTY, C.PICK_carton PICKCTN, C.CARTON_QTY - C.PICK_CARTON RESTCTN
                     ...
                     WHERE C.PALLET_NO = :palletId ORDER BY 
										 case when PICK_STATUS ='FP' then PICK_STATUS  end desc, 
                                            case when PICK_STATUS !='FP' then udt  end desc";
        ...
        public IHttpActionResult GetTaskStockLoc(string palletId, string partNo)
        {
            PPSDb ppsDb = new PPSDb();
            string sql = "SELECT DISTINCT ICTPN from ppsuser.T_PALLET_ORDER where PALLET_NO=:palletId";
            if (!String.IsNullOrWhiteSpace(partNo) && partNo.ToUpper() != "ALL")
                sql += " and ictpn = :partNo";
            else
                sql += " and rownum = 1";
            var list = ppsDb.Query<string>(sql, new { palletId = palletId, partNo = partNo }).ToArray();
            ...
            foreach (var ictpnn in list)
            {
                ...
                list1 = list1.OrderBy(x => x.ICTPN).ThenBy(x => x.udt).ToList();
            }
        ...
        public IHttpActionResult PickCTN(PickCTNViewModel vm)
        {
            ...
            {
                return Ok(Result.Create(false, "" + errmsg1.TP()));
            ...
            {
                return Ok(Result.Create(false, "" + errmsg2.TP()));
            }
            ...
            if (!vDBType.outparavalue.Equals("TEST"))
            {
                if (vMarinaPackoutFlag.outmarinaflag.Equals("Y"))
                {
                    ...
                    if (!bl)
                    {
                        return Ok(Result.Create(false, "MarinaServer " + msgCheckMarina.TP()));
                    }
                }
                ...
                if (vMarinaPackoutFlag.outpackoutflag.Equals("Y"))
                {
                    ...
                    if (!bl)
                    {
                        return Ok(Result.Create(false, "PackoutLogic " + msgPackout.TP()));
                    }
                }
            }
            ...
            var m3 = ppsDb.InsertPalletPick(IsEntirety, vm.PickPalletNo, vm.Palletid, strCarton, vm.empNo);
            string errmsg3 = m3.ErrMsg;
            if (!errmsg3.Equals("OK") && !errmsg3.StartsWith("FINISH"))
            {
                return Ok(Result.Create(false, errmsg3.TP()));
            }
            else
            {
            ...
            res.Message = errmsg3;
            res.Anything = m3;
            return Ok(Result.Create(true, res.Message, JsonConvert.SerializeObject(res.Anything)));
        }
        ...
        [HttpGet]
        [Route("GetPickLogByDate")]
        public IHttpActionResult GetPickLogByDate(string uuid, DateTime pickDate)
        {
            PPSDb ppsDb = new PPSDb();
            ...
            string sql = @" 
                      ...
                      join (select a.shipment_id ShipmentId,
                        ...
                        on aa.shipmentid = bb.ShipmentId
                       and aa.palletno = bb.palletno
                       and  trunc(aa.starttime) = :pickDate
                        join ( select distinct pallet_no, emp_no from ppsuser.T_PALLET_PICK) d
                                on aa.palletno= d.pallet_no
                         group by aa.shipmentid,
                                       aa.palletno,
                                       ...
                                       bb.DealQty,
                                 d.EMP_NO";
            var list = ppsDb.Query<PPSPickLogModel>(sql, new { uuid, pickDate });
            return Ok(Result.Create(true, "", JsonConvert.SerializeObject(list)));

        }

        [HttpGet]
        [Route("PrintPickLabel")]
        public IHttpActionResult PrintPickLabel(string cartonNo)
        {
            ExecutionResult res = new ExecutionResult();
            res.Status = true;
            PPSDb ppsDb = new PPSDb();
            string ctn = DelPrefixCartonSN(cartonNo);
            string pickPallet = ppsDb.changeSNtoPickPalletno(ctn);
            if (String.IsNullOrWhiteSpace(pickPallet))
            {
                res.Status = false;
            ...
            if (res.Status)
            {
                var palletInfo = ppsDb.GetPickLabelInfo(pickPallet);
                if (palletInfo.Status)
                {
                    var arrParamPallet = palletInfo.Anything.ToString().Split(',');
                    res.Anything = new
                    {
                        ShipmentID = arrParamPallet[0],
                        PickPalletNo = arrParamPallet[1],
                        carrierName = arrParamPallet[2],
                        Qty = arrParamPallet[3],
                        PalletType = arrParamPallet[4],
                        CartonQtyInfo = arrParamPallet[5],
                        PalletNumber = arrParamPallet[6],
                        Remark = arrParamPallet[7],
                        ShipmentType = arrParamPallet[8],
                    };
                }
                else
                {
                    res.Status = false;
                    res.Message = palletInfo.Message;
                }
            }

            return Ok(Result.Create(res.Status, res.Message, JsonConvert.SerializeObject(res.Anything)));
        }
    }
}
